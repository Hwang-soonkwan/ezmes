<template>
    <div>
        <h1 style = "margin-left:4.5%; margin-top:-10px;">WorkOrder</h1>
        <v-col style="margin-bottom:40px;">
            <div class="text-center">
                ...
                >
                    <WorkOrder :offline="offline" class="video-card" :isNew="true" :editMode="true" v-model="newValue" 
        ...
        <v-row>
            <WorkOrder :offline="offline" class="video-card" v-for="(value, index) in values" v-model="values[index]" v-bind:key="index" @delete="remove"/>
        </v-row>
    </div>
</template>
